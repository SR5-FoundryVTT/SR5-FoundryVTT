...
export class ChummerImportForm extends FormApplication {
    ...
    doStuff() {
        ...
        console.log(`SR5 - ChummerImportForm |`, chummerfile);
        ...
        if (chummerfile.characters?.character) {
            ...
            try {
                this.assign(updateData, 'name', c.alias);
                this.assign(update, 'player_name', c.playername);
                this.assign(update, 'name', c.alias);
                this.assign(update, 'metatype', c.metatype);
                this.assign(update, 'sex', c.sex);
                this.assign(update, 'age', c.age);
                this.assign(update, 'street_cred', c.calculatedstreetcred);
                this.assign(update, 'notoriety', c.calculatednotoriety);
                this.assign(update, 'public_awareness', c.calculatedpublicawareness);
                this.assign(update, 'nuyen', parseInt(c.nuyen.replace(',', '')));
            ...
            atts.forEach((att) => {
                try {
                    const newAtt = this.parseAtt(att.name);
                    ...
                    if (s.rating > 0 && s.islanguage) {
                        ...
                        if (skill) {
                            ...
                            if (s.skillspecializations) {
                                skill.specs = this.toArray(s.skillspecializations.skillspecialization.name);
                            }
            ...
            if (qualities && c.qualities && c.qualities.quality) {
                const qualities = this.toArray(c.qualities.quality);
                qualities.forEach((q) => {
                    try {
                        const data = {};
            ...
            if (weapons && c.weapons != null && c.weapons.weapon != null) {
                const weapons = this.toArray(c.weapons.weapon);
                weapons.forEach((w) => {
                    try {
                        const data = {};
                        const action = {};
                        ...
                        if (w.type.toLowerCase() === 'melee') {
                            ...
                            //     range.mods = [];
                            //     const accessories = this.toArray(w.accessories.accessory);
            ...
            if (armor && c.armors && c.armors.armor) {
                const armors = this.toArray(c.armors.armor);
                armors.forEach((a) => {
                    try {
                        const data = {};
                        const armor = {};
                        ...
                        if (a.armormods && a.armormods.armormod) {
                            ...
                            const modDesc = [];
                            const mods = this.toArray(a.armormods.armormod);
                            mods.forEach((mod) => {
            ...
            if (cyberware && c.cyberwares && c.cyberwares.cyberware) {
                const cyberwares = this.toArray(c.cyberwares.cyberware);
            ...
            if (powers && c.powers && c.powers.power) {
                const powers = this.toArray(c.powers.power);
                powers.forEach((p) => {
                    const data = {};
                    if (p.description)
                        data.description = {
            ...
            if (equipment && c.gears && c.gears.gear) {
                const gears = this.toArray(c.gears.gear);
                gears.forEach((g) => {
                    try {
                        const data = {};
                        let {
            ...
            if (spells && c.spells && c.spells.spell) {
                const spells = this.toArray(c.spells.spell);
                spells.forEach((s) => {
                    try {
        ...
        ChummerImportForm.saveData(this, updateData, items);
    }

    /**
     * converst the value to an array if it is not already one
     * @param {*} value 
     */
    ...
    /**
     * assigns the value to the target if it is not undefined
     * @param {*} target 
     * @param {*} value 
     */
    assign = function(target, property, value) {
        if(value) {
    ...
    /**  
     * Returns the full attribute name based on the chummer shortcut
     * @private
     * 
     * @param {String} att The 3 character shortcut
     */
    parseAtt = function(att) {
    ...
    static async saveData(sender, updateData, items) {
        await sender.object.update(updateData);
        await sender.object.createEmbeddedEntity('OwnedItem', items);
