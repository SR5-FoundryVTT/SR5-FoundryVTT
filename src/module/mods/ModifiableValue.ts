...
export class ModifiableValue<Field extends ModifiableValueType = ModifiableValueType> {
     ...
     * @param {number} [options.min] - If provided, enforces a minimum value (adds an `SR5.Tests.Labels.EnforcedMinimum` change).
     * @param {number} [options.max] - If provided, enforces a maximum value (adds an `SR5.Tests.Labels.EnforcedMaximum` change).
    ...
    calcTotal(options?: { min?: number; max?: number, decimal?: boolean }): number {
        ...
        this.remove('SR5.Tests.Labels.EnforcedMaximum');
        this.remove('SR5.Tests.Labels.EnforcedMinimum');
        ...
        if (options?.max != null && this._field.value > options.max) {
            ...
            this.addUnique('SR5.Tests.Labels.EnforcedMaximum', options.max, { type: 'downgrade', priority: ModifiableValue.TOP_PRIORITY });
            this._field.value = options.max;
        }

        if (options?.min != null && this._field.value < options.min) {
            this._markPreviousChangesMasked(this._field.changes.length);
            this.addUnique('SR5.Tests.Labels.EnforcedMinimum', options.min, { type: 'upgrade', priority: ModifiableValue.TOP_PRIORITY });
