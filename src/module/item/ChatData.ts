...
export const ChatData = {
    call_in_action: (system: Shadowrun.CallInActionData, labels, props) => {
        switch (system.actor_type) {
            case 'sprite':
                if (system.sprite.type) props.push(`${localize("SR5.Compilation.SpriteType")} ${localize(SR5.spriteTypes[system.sprite.type])}`);
                if (system.sprite.level) props.push(`${localize('SR5.Level')} ${system.sprite.level}`);
                return;
            case 'spirit':
                if (system.spirit.type) props.push(`${localize("SR5.Summoning.SpiritType")} ${localize(SR5.spiritTypes[system.spirit.type])}`);
                if (system.spirit.force) props.push(`${localize('SR5.Force')} ${system.spirit.force}`);
                return;
        }
    },
    action: (system, labels, props) => {
        if (system.action) {
            ...
            if (system.action.mod) {
                labelStringList.push(`${localize('SR5.ItemMod')} (${system.action.mod})`);
            }
            ...
            if (system.action.limit) {
                ...
                const attribute = limit.attribute ? `${localize(SR5.limits[limit.attribute])}` : '';
            ...
            if (system.action.damage.type.value) {
                ...
                let attribute = damage.attribute ? `${localize(SR5.attributes[damage.attribute])} ${operator} ` : '';
                ...
                attribute = ap.attribute ? `${localize(SR5.attributes[ap.attribute])} ${operator} ` : '';
    ...
    contact: (system, labels, props) => {
        ...
        props.push(`${localize('SR5.Connection')} ${system.connection}`);
        props.push(`${localize('SR5.Loyalty')} ${system.loyalty}`);
        if (system.blackmail) {
            props.push(`${localize('SR5.Blackmail')}`);
        }
        if (system.family) {
            props.push(localize('SR5.Family'));
        }
    },
    ...
    ammo: (system, labels, props) => {
        if (system.damageType) props.push(`${localize("SR5.DamageType")} ${system.damageType}`);
        if (system.damage) props.push(`${localize("SR5.DamageValue")} ${system.damage}`);
        if (system.element) props.push(`${localize("SR5.Element")} ${system.element}`);
        if (system.ap) props.push(`${localize("SR5.AP")} ${system.ap}`);
        if (system.blast.radius) props.push(`${localize('SR5.BlastRadius')} ${system.blast.radius}m`);
        if (system.blast.dropoff) props.push(`${localize('SR5.Dropoff')} ${system.blast.dropoff}/m`);
    },

    program: (system, labels, props) => {
        props.push(localize(SR5.programTypes[system.type]));
    },
    ...
    quality: (system, labels, props) => {
        ...
        props.push(Helpers.label(system.type));
        if (system.rating) props.push(`${localize('SR5.Rating')} ${system.rating}`);
    },
    ...
    critter_power: (system, labels, props) => {
        props.push(localize(SR5.critterPower.types[system.powerType]));
        props.push(localize(SR5.critterPower.durations[system.duration]));
        props.push(localize(SR5.critterPower.ranges[system.range]));
        if (system.rating) props.push(`${localize('SR5.Rating')} ${system.rating}`);
    ...
    weapon: (system, labels, props, item: SR5Item) => {
        ...
        if (equippedAmmo && system.ammo && system.ammo.current?.max) {
            if (equippedAmmo) {
                ...
                if (ammoData.blast.radius) props.push(`${localize('SR5.BlastRadius')} ${ammoData.blast.radius}m`);
                if (ammoData.blast.dropoff) props.push(`${localize('SR5.Dropoff')} ${ammoData.blast.dropoff}/m`);
                if (spare_clips && spare_clips.max) props.push(`${localize('SR5.SpareClips')} (${spare_clips.value}/${spare_clips.max})`);
            }
        }

        if (system.technology?.conceal?.value) {
            props.push(`${localize('SR5.Conceal')} ${system.technology.conceal.value}`);
        }

        if (system.category === 'range') {
            if (system.range.rc) {
                let rcString = `${localize('SR5.RecoilCompensation')} ${system.range.rc.value}`;
                if (item?.actor) {
                    rcString += ` (${localize('SR5.Total')} ${item.totalRecoilCompensation})`;
                }
                props.push(rcString);
            }
            if (system.range.modes) {
                ...
                props.push(newModes.map((m) => localize(m)).join('/'));
            }
            ...
            if (system.melee.reach) {
                const reachString = `${localize('SR5.Reach')} ${system.melee.reach}`;
            ...
            if (blast?.radius) props.push(`${localize('SR5.BlastRadius')} ${blast.radius}m`);
            if (blast?.dropoff) props.push(`${localize('SR5.Dropoff')} ${blast.dropoff}/m`);
