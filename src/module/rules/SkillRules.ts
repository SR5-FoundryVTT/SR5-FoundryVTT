...
export class SkillRules {
     ...
     */
    static mustDefaultToRoll(skill: SR5Item<'skill'>): boolean {
        console.error('TODO: tamif - use value instead of rating');
        return skill.system.skill.rating === 0;
    }
    /**
    ...
    static allowDefaultingRoll(skill: SR5Item<'skill'>): boolean {
        ...
        // still defaulting.
        return skill.system.skill.defaulting;
    }
     ...
     */
    static level(skill: SR5Item<'skill'>, options = {specialization: false}): number {
    ...
    static injectSkills(names: string[], source: SR5Actor, rollData: SR5Actor['system'], options: { bigger: boolean }) {
        ...
        for (const name of names) {
            ...
            // if (options.bigger) {
            //     targetSkills[name] = sourceSkill.system.skill.rating > targetSkill.value ? sourceSkill : targetSkill;
            // } else {
