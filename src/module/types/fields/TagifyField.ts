...
type TagifyList = { id: string; label: string }[];
...
export class TagifyField<
    ...
    const PersistedType extends PersistedElementType[] | null | undefined = ArrayField.PersistedType<PersistedElementType, Options>,
> extends foundry.data.fields.ArrayField<ElementFieldType, Options, AssignmentElementType, InitializedElementType, AssignmentType, InitializedType, PersistedElementType, PersistedType> {
     ...
     */
    override _toInput(config: DataField.ToInputConfig<InitializedType> & { options?: TagifyList, value?: TagifyList }): HTMLElement {
        const optionsMap = new Map(
            config.options?.map(opt => [opt.id, opt.label]) ?? []
        );
